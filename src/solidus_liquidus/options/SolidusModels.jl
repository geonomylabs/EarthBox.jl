        ...
        option_ids[option_names.NoMelting] => 
            OptionState(
                ...
                description="Negative one is used to signal a material without melting. " *
                           "This is the default value for itype_solidus."
            ),
        option_ids[option_names.SedimentGerya2010] => 
            OptionState(
                option_name=string(option_names.SedimentGerya2010),
                description="Sediment solidus model from [gerya2010](@citet) Table 17.2."
            ),
        option_ids[option_names.UprContCrustGerya2010] => 
            OptionState(
                option_name=string(option_names.UprContCrustGerya2010),
                description="Upper continental crust solidus model from [gerya2010](@citet) Table 17.2."
            ),
        option_ids[option_names.LwrContCrustGerya2010] => 
            OptionState(
                option_name=string(option_names.LwrContCrustGerya2010),
                description="Lower continental crust solidus model from [gerya2010](@citet) Table 17.2."
            ),
        option_ids[option_names.DryMantleGerya2010] => 
            OptionState(
                option_name=string(option_names.DryMantleGerya2010),
                description="Dry mantle solidus model from [gerya2010](@citet) Table 17.2."
            ),
        option_ids[option_names.PeridotiteKatz2003] => 
            OptionState(
                option_name=string(option_names.PeridotiteKatz2003),
                description="Peridotite solidus model from [katz03](@citet)."
            ),
        option_ids[option_names.GabbroGerya2010] => 
            OptionState(
                option_name=string(option_names.GabbroGerya2010),
                description="Gabbro solidus model from [gerya2010](@citet) Table 17.2."
            ),
        option_ids[option_names.GabbroSchubert2013] => 
            OptionState(
                option_name=string(option_names.GabbroSchubert2013),
                description="Gabbro solidus model from Schubert et al. (2013)"
            ),
        option_ids[option_names.GabbroGlacier] => 
            OptionState(
                option_name=string(option_names.GabbroGlacier),
                description="Gabbro solidus model that approximates the gabbro " *
                           "glacier model of [maclennan04](@citet)."
            )
    )
end

function format_solidus_options()
    solidus_options = get_options()
    sorted_ids = sort(collect(keys(solidus_options)))
    
    # Create header
    header = "| `stype_solidus` | `itype_solidus` | Description |\n"
    separator = "|:--------------|:-------------:|:------------|\n"
    
    # Create rows
    rows = join(
        ["| `\"$(solidus_options[id].option_name)\"` | $(id) | " *
         "$(solidus_options[id].description) |" 
