#!/usr/bin/env julia
# -*- coding: utf-8 -*-
""" Calculate visco-elasticity factor used in Stokes equations.
"""
module Viscoelastic

""" Calculate visco-elasticity factor from scalar grid arrays.
"""
function calc_viscoelastic_factor(
    i::Int,
    j::Int,
    viscosity::Matrix{Float64},
    shear_modulus::Matrix{Float64},
    timestep::Float64
)::Float64
    return calculate_viscoelastic_factor(
        viscosity[i, j], shear_modulus[i, j], timestep)
end
...
""" Print warning message if denominator is zero.
"""
