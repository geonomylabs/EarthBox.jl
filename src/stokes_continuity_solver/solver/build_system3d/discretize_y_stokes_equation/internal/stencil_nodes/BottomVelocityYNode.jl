...
function coefficients_and_rhs_term(
    ...
    if !basic_cell_in_last_two_bottom_rows(i, ynum)
        ivyD = ivy + 4
        Lv[inz] = 2.0 * etan[i+1, j, k] / ystp[i+1] / ystpc[i+1]
        Li[inz] = ivy
        Lj[inz] = ivyD
        Lii[inz] = i
        Ljj[inz] = j
        ...
        # vyD is a lower ghost boundary node (or its central node is
        # prescribed).
        Lv[inz_c] = (
            Lv[inz_c] +
            bbottomy[j+1, k+1, 2] * 2.0 * etan[i+1, j, k] / ystp[i+1] / ystpc[i+1]
        )
        R[ivy] = (
            R[ivy] -
            bbottomy[j+1, k+1, 1] * 2.0 * etan[i+1, j, k] / ystp[i+1] / ystpc[i+1]
        )
    end
    return inz
end
