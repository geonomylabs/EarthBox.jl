...
""" Plot marker extracted meltfrac.

Plots extracted melt fraction markers with optional contours based on the 
provided parameters and marker arrays. Filters markers to only show those 
with mantle melting material IDs.
...
"""
function plot_filtered_extracted_meltfrac(
    parameters::PlotParameters,
    marker_arrays::PlotMarkerArrays,
    materials::Materials,
    axes::AxesType
)::Nothing
    plot_marker_scalars = parameters.marker_plot_params.plot_extracted_meltfrac
    plot_contours = parameters.marker_plot_params.plot_extracted_meltfrac_contours

    marker_scalar_array = marker_arrays.marker_extracted_meltfrac
    label = "Extracted Melt Fraction"
    ...
    if plot_marker_scalars == 1 || plot_contours == 1
        println(">> Plotting extracted meltfrac")
