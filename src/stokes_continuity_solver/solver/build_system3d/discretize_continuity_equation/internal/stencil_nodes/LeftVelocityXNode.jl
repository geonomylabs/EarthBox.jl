...
function coefficients_and_rhs_term(
    ...
    if !basic_cell_along_left_boundary(j)
        ...
        inz_xL = inz
        inz = inz + 1
        # If vxR is the right-boundary ghost, substitute its boundary
        # equation in terms of vxL using brightx.
        if basic_cell_along_right_boundary(j, xnum)
            Lv[inz_xL] = Lv[inz_xL] + brightx[i+1, k+1, 2] * pscale / xstp[j]
            R[ipr] = R[ipr] - brightx[i+1, k+1, 1] * pscale / xstp[j]
        end
    end
    return inz
end
