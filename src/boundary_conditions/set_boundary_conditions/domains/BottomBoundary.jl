...
function free_slip!(model::ModelData)::Nothing
    FreeSlipBC.set_free_slip_bc_at_bottom_boundary!(model)
    return nothing
end

function no_slip!(model::ModelData)::Nothing
    NoSlipBC.set_no_slip_bc_at_bottom_boundary!(model)
    return nothing
end

function inflow_outflow!(model::ModelData, vy::Float64)::Nothing
    InflowOutflowBC.set_inflow_outflow_bc_at_bottom_boundary!(model, vy)
    return nothing
end

function temperature!(
    model::ModelData, 
    t_bottom::Union{Float64, Nothing}=nothing
)::Nothing
    if t_bottom === nothing
        t_bottom = SetBCTools.get_bottom_temperature(model)
    end
    TemperatureBC.set_temperature_at_bottom_boundary!(t_bottom, model)
    return nothing
end
...
Set constant vertical temperature gradient at bottom boundary.

Temperature is assumed to increase with depth starting at the basic
nodes above the bottom boundary. Temperature increases linearly with
...
function constant_vertical_temperature_gradient!(
    ...
    ConstantGradientBC.set_constant_vertical_gradient_at_bottom_boundary!(
        ystp, dtdy, model)
    return nothing
end

end # module BottomBoundary 