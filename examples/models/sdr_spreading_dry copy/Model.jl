...
const ROOT_PATH_OUTPUT = "/mnt/extradrive1/earthbox_output/sdr_spreading_dry"
const ROOT_PATH_STORAGE = "/mnt/extradrive2/earthbox_output/sdr_spreading_dry"

# Get the input parameters object so names can be accessed programmatically
const PARAMS = get_eb_parameters()

# Constants
const tolerance_picard              = 1e-3 
...
const temperature_base_lith_celsius = 1345.0 # C, this is the "normal" temperature at the base of the lithosphere

# Derived Constants
const temperature_bottom_celsius = 
    (
      temperature_base_lith_celsius 
    + adiabatic_gradient * (ysize - thick_air - thick_lith) / 1000.0
    ) # This is the normal temperature at the bottom of the model
...
function initialize_boundary_conditions(
    ...
    BoundaryConditions.Temperature.initialize!(
        ...
        temperature_bottom = ConversionFuncs.celsius_to_kelvin(temperature_bottom_celsius + delta_temperature_transient)
    )
    BoundaryConditions.Velocity.initialize!(
        model, full_velocity_extension = case_parameters[PARAMS.full_velocity_extension.name].value)
    return nothing
end
...
function initialize_marker_temperature(eb::EarthBoxState)::Nothing
    Markers.MarkerTemperature.initialize!(
        ...
        parameters=Dict(
            PARAMS.amplitude_perturbation.name      => 5000.0,
...
function initialize_stokes_continuity_solver(
    ...
    GlobalPlasticityLoop.initialize!(
        ...
        tolerance_picard       = tolerance_picard,
        nglobal                = 100,
    )
    return nothing
end
...
function initialize_surface_processes_model(
    ...
    SurfaceProcesses.Sealevel.initialize!(
        ...
        base_level_shift_end_time = 50.0, # Myr
