...
function make_jld2_file(markers_xdmf::MarkersXdmfTimeStep, output_dir::String)
    ...
    JLD2.jldopen(jld_marker_file_path, "w"; compress=ZlibCompressor(level=1)) do file
        ...
        let
            ...
            if nmarkers <= typemax(UInt32)
                group["array"] = collect(UInt32, 0:nmarkers-1)
            else
                @warn(
                    "nmarkers ($nmarkers) exceeds UInt32 limit; " *
                    "storing marker_ids as Int64. If using ParaView, " *
                    "the XDMF DataItem in XdmfParts.jl must be updated " *
                    "to Precision=\"8\" to match, otherwise ParaView " *
                    "will misread the bytes.",
                    maxlog=1
                )
                group["array"] = collect(Int64, 0:nmarkers-1)
            end
