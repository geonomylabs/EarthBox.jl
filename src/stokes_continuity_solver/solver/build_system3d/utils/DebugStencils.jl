...
function check_large_matrix_x_stokes(
    ...
    if inz == TARGET_INZ
        ...
        println(
            "Large Matrix X-Stokes $loc_id: inz = $inz, Lv = $Lv, ",
            "Lii = $Lii, Ljj = $Ljj, i = $i, j = $j, k = $k, ",
            "ivx = $ivx, ivy = $ivy, ivz = $ivz, ipr = $ipr"
        )
    end
    return nothing
end
...
function check_inz_value_z_stokes_internal(inz::Int64, loc_id::Int64)::Nothing
    if inz == TARGET_INZ
        println("Z-Stokes Internal $loc_id: inz = $inz")
    end
    return nothing
end

function check_inz_value_z_stokes_boundary(inz::Int64, loc_id::Int64)::Nothing
    if inz == TARGET_INZ
        println("Z-Stokes Boundary $loc_id: inz = $inz")
    end
    return nothing
end
...
function check_large_matrix_in_stencil(
    ...
    if inz == TARGET_INZ
        println(
            " $name inz = $inz, Lv[$inz] = $(Lv[inz]), i = $i, j = $j, k = $k, ",
            "etan = $etan, xstp = $xstp, xstpc = $xstpc"
        )
    end
    return nothing
end
