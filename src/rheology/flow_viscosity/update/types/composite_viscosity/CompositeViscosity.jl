...
function update_marker_flow_viscosity!(
    ...
    use_strain_softening::Bool = iuse_viscous_strain_soft == 1

    @inbounds Threads.@threads for imarker in 1:marknum
        ...
        elseif flow_type == 1 || flow_type == 2
            ...
            pressure_megapascals = marker_pr[imarker]*PaToMPa
            strain_rate_ratio    = marker_sr_ratio[imarker]
            temperature          = max(marker_TK[imarker], temperature_top)
            ...
            if use_strain_softening
                pre_exponential_dislocation = apply_strain_softening(
                    vsoftfac, pre_exponential_dislocation,
                    strain, strain_initial, strain_final
                )
            end
            stress_invariant_initial = calc_initial_stress_invariant(
                stress_xx, stress_xy, stress_minimum)
            ...
            (
                ...
                pressure_megapascals, temperature, gas_constant,
                pre_exponential_dislocation, activation_energy_dislocation,
                activation_volume_dislocation, stress_exponent_dislocation,
                pre_exponential_peierls, stress_exponent_peierls_m,
                stress_exponent_peierls_n, stress_peierls,
                pre_exponential_diffusion, activation_energy_diffusion,
                activation_volume_diffusion, 
                stress_invariant_initial*PaToMPa,
                false, # use_old_term
            ...
            stress_invariant_forecast = forecast_viscoelastic_stress(
                timestep, shear_modulus, strain_rate_ratio,
                stress_xx, stress_xy, strain_rate_xx, strain_rate_xy,
                stress_minimum, viscosity
            )
            ...
            while icount < max_iterations && delta_stress > 1.0
                ...
                (
                    ...
                    stress_invariant_initial, stress_invariant_forecast,
                    stress_invariant_avg, stress_invariant_forecast_update
                )
                delta_stress = abs(stress_invariant_forecast - stress_invariant_initial)
            end
            viscosity = clamp(viscosity, viscosity_minimum, viscosity_maximum)
        end
...
@inline function apply_strain_softening(
    ...
    return update_pre_exponential_for_viscous_strain_softening(
            pre_exponential_dislocation, strain, vsoftfac, 
...
@inline function forecast_viscoelastic_stress(
    ...
    fac1 = 2.0 * viscosity * strain_rate_ratio
    one_minus_ve_fac = 1.0 - visco_elastic_factor
    stress_xx_new = fac1 * strain_rate_xx * one_minus_ve_fac + stress_xx * visco_elastic_factor
    stress_xy_new = fac1 * strain_rate_xy * one_minus_ve_fac + stress_xy * visco_elastic_factor
...
@inline function second_stress_invariant2d(stress_xx::Float64, stress_xy::Float64)::Float64
    return sqrt(stress_xx*stress_xx + stress_xy*stress_xy)
end
...
@inline function update_bisection_limits(
    ...
    if (stress_invariant_forecast > stress_invariant_initial && stress_invariant_forecast_update > stress_invariant_avg) ||
       (stress_invariant_forecast < stress_invariant_initial && stress_invariant_forecast_update < stress_invariant_avg)
