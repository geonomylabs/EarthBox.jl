...
function initialize!(model::ModelData)::Nothing
    ...
    distance_from_base_lith_to_bottom_km = (ysize - thick_air - thick_lithosphere)/1000.0

    (
        adiabatic_temperature_gradient_celsius_km
    ) = model.heat_equation.parameters.initial_condition.adiabatic_gradient.value

    temperature_bottom = model.bcs.parameters.temperature.temperature_bottom.value
    temperature_asthenosphere = (
        temperature_bottom
        - distance_from_base_lith_to_bottom_km
        * adiabatic_temperature_gradient_celsius_km
    )
...
function print_info(
    distance_from_base_lith_to_bottom_km::Float64,
    ...
    println(
        ">>> temperature_bottom (C): ",
        kelvin_to_celsius(temperature_bottom)
    )
    println(
        ">>> temperature_asthenosphere (C): ",
        kelvin_to_celsius(temperature_asthenosphere)
    )
    return nothing
end
